...
- When more than one process, timer, or workflow can share a provider quota, atomically reserve capacity immediately before every provider call. A separate non-atomic availability check is not sufficient.
- A provably single consumer must still check the available hard limit immediately before every provider call.
...
3. concurrent consumers sharing one provider quota;
4. day/week rollover;
5. provider throttle/error;
6. quota exhaustion before the next provider call;
7. persistent timer catch-up;
8. last-known-good behavior after refresh failure.
