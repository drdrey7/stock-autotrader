...
CURRENT_ASSETS_LABELS = ("Current Assets",)
CURRENT_LIABILITIES_LABELS = ("Current Liabilities",)
...
DEBT_NONCURRENT_FACT_CONCEPTS = ("us-gaap:LongTermDebtNoncurrent",)
...
ANNUAL_FILING_FORMS = ["10-K", "10-K/A", "20-F", "20-F/A"]
...
def _latest_value(row: dict[str, Any], preferred_period: str | None = None) -> float | None:
    ...
    if preferred_period is not None:
        return _finite(row[preferred_period]) if preferred_period in row else None
    for key in keys:
...
def _annual_period_count(filings: Any) -> int | None:
    """Count unique annual periods from EdgarTools filing-index metadata."""
    try:
        annual = filings.filter(form=ANNUAL_FILING_FORMS, amendments=True)
        values = annual.data["period_of_report"].to_pylist()
    except Exception:
        return None
    periods = {_date_value(value) for value in values}
...
def fetch_latest_filing_metadata(symbol: str, identity: str) -> FilingMetadata:
    ...
    for field in ("accession_number", "accession_no", "accession"):
        ...
        if isinstance(value, str) and value.strip():
            ...
            break
    period = _date_value(getattr(filing, "period_of_report", None))
    ...
    filed_date = _date_value(
        ...
        or getattr(filing, "filed_date", None)
        or getattr(filing, "acceptance_datetime", None),
    )
    annual_periods_available = None
    try:
        annual_periods_available = _annual_period_count(filings)
    except Exception:
        pass
    return FilingMetadata(accession, period, form, filed_date, annual_periods_available)
...
def fetch_accounting_inputs(symbol: str, identity: str, filing: FilingMetadata | None = None) -> AccountingInputs:
    ...
    if not income_rows or not cash_rows or not balance_rows:
    ...
    shares = _share_fact_value(
        ...
        SHARES_FACT_CONCEPTS,
        (getattr(filing, "filed_date", None) or filing.period_of_report) if filing else None,
    )
    operating_cash_flow = _find(cash_rows, OCF_LABELS, income_period if isinstance(income_period, str) else None)
    ...
    return AccountingInputs(
        ...
        current_assets=_balance_value(balance_rows, CURRENT_ASSETS_CONCEPTS, CURRENT_ASSETS_LABELS, balance_period_name),
        current_liabilities=_balance_value(balance_rows, CURRENT_LIABILITIES_CONCEPTS, CURRENT_LIABILITIES_LABELS, balance_period_name),
...
def fetch_annual_fundamentals(
    ...
    for year in _annual_years(income):
        ...
        shares = _share_fact_value(facts + share_facts, SHARES_FACT_CONCEPTS, fiscal_year=year)
        ...
        rows.append(AnnualFundamental(
            ...
            shares_outstanding=shares,
            current_assets=_balance_value(balance_rows, CURRENT_ASSETS_CONCEPTS, CURRENT_ASSETS_LABELS, period),
            current_liabilities=_balance_value(balance_rows, CURRENT_LIABILITIES_CONCEPTS, CURRENT_LIABILITIES_LABELS, period),
            as_of=period,
        ))
    return rows
