 ...
 *   - quote week == L      -> basis = sum_199 - anchor_close
 *     (the quote's own week was already stored as completed history)
...
export function computeLiveSma200w(
  ...
  if (delta === 1) {
    ...
    // The quote's own week is already stored as completed history: exclude
    // the anchor week from the basis so it is counted exactly once (via the
    // live quote, as the 200th term).
    basis = metrics.sum_199 - metrics.anchor_close;
  } else {
