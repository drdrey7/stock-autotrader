...
function EarningsCalendar({ period, setPeriod, onSelect }: { period: CalendarPeriod; setPeriod: (period: CalendarPeriod) => void; onSelect: (e: EarningsCompany) => void }) {
  ...
  return <Card className="calendar-card"><div className="calendar-head"><div><span className="eyebrow">MONTHLY CALENDAR</span><h2>{monthName} {year}</h2></div><div><button aria-label="Previous month" onClick={() => moveMonth(-1)}><ChevronLeft/></button><button className="today" onClick={goToday}>Today</button><button aria-label="Next month" onClick={() => moveMonth(1)}><ChevronRight/></button></div></div><div className="weekdays">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d => <span key={d}>{d}</span>)}</div><div className="calendar-grid">{days.map((day, index) => { const date = day ? `${year}-${String(month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}` : ""; const reports = earnings.filter(e => e.scheduledDate === date); const calendarDateLabel = day ? new Intl.DateTimeFormat("en", { weekday: "long", month: "long", day: "numeric", year: "numeric" }).format(dateFromKey(date)) : ""; return <div key={index} className={!day ? "empty" : date === todayKey ? "is-today" : ""}>{day && <><span className="day-number">{day}</span><div className="calendar-events">{reports.map(e => <button key={e.id} aria-label={`${e.symbol} ${e.company}, ${eventLabel(e)}, ${calendarDateLabel}`} onClick={() => onSelect(e)} title={`${e.company} · ${eventLabel(e)}`}><i style={{ "--company": e.color } as React.CSSProperties}/><b>{e.symbol}</b><small>{eventLabel(e)}</small></button>)}</div></>}</div>; })}</div></Card>;
}
...
export default function EarningsCalendarPage({ onSelect }: { onSelect: (e: EarningsCompany) => void }) {
  ...
  const thisWeekStartDate = new Date(today); thisWeekStartDate.setDate(today.getDate() - mondayBasedWeekday(today));
