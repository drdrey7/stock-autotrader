...
class Ingestor:
    ...
    def __init__(self, settings: Settings, symbols: list[str], d1: D1Client,
                 clock: Callable[[], dt.datetime] | None = None,
                 accept_trade_fn: Callable[[dt.datetime, dt.datetime], bool] | None = None,
                 close_checkpoint: CloseCandidateCheckpoint | None = None) -> None:
        ...
        self.parser = TradeFrameParser(symbols, now_fn=lambda: int(self.clock().timestamp() * 1000))
    ...
    def on_message(self, raw: str) -> None:
        ...
        try:
            result = self.parser.parse(raw, self.settings.max_timestamp_future_seconds,
        ...
        for tick in result.ticks:
            ...
            # Close-window progress is only allowed into rollover-capable RAM
            # state after its crash-safe local proof is durable. A filesystem
            # failure therefore drops only this candidate; later eligible ticks
            # retry, and unrelated symbols continue ingesting normally.
            if is_close_baseline_candidate(tick.timestamp_ms) and not self._checkpoint_first_close_tick(tick):
                continue
    ...
    def _checkpoint_first_close_tick(self, tick) -> bool:
    ...
    def flush_once(self, now: dt.datetime | None = None) -> None:
        ...
        if self.settings.log_flush_summaries:
            log_event("flush", requested=len(rows), written=len(result.written), failed=len(result.failed),
                      duration_ms=duration_ms, http_status=result.http_status or None,
                      error=result.error, market="open")
        self._write_health_record()
    ...
    def _watchdog_tick(self) -> None:
        ...
        with self._watchdog_lock:
            ...
            if self._grace_mono is not None and mono - self._grace_mono < self.settings.ws_market_stall_seconds:
                return
            if (self._last_stall_reconnect_mono is not None
            ...
            if last_tick_mono is None:
                ...
                return
            if mono - last_tick_mono >= self.settings.ws_market_stall_seconds:
                self._trigger_stall_reconnect(ws, mono)
    ...
    def _trigger_stall_reconnect(self, ws: FinnhubWebSocketClient, mono: float) -> None:
        ...
        logger.warning(json.dumps({
            "event": "ws_market_data_stalled", "stall_seconds": round(stall_seconds, 1),
            "subscriptions": ws.subscriptions_sent, "message_count": self.health.message_count,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    log_event("startup", version=__version__, finnhub_key_configured=secret_present("FINNHUB_API_KEY"),
    ...
    d1 = D1Client(settings.cloudflare_api_token, settings.cloudflare_account_id,
                  settings.cloudflare_d1_database_id, max_retries=settings.d1_max_retries,
                  retry_base_seconds=settings.d1_retry_base_seconds,
                  request_timeout_seconds=settings.d1_request_timeout_seconds)
    close_checkpoint = (CloseCandidateCheckpoint(settings.close_candidate_state_path, symbols)
                        if settings.close_candidate_state_path is not None else None)
    ingestor = Ingestor(settings, symbols, d1, close_checkpoint=close_checkpoint)
    ingestor.baseline_read()
    ws = FinnhubWebSocketClient(settings, symbols, on_message=ingestor.on_message,
