...
For AI-generated changes that affect runtime behavior, user-facing product behavior,
persistence, quotas, provider APIs, retries, timers, background jobs, authentication,
deployment, or production infrastructure, reviewers must also verify the invariants
in `.github/AI_ENGINEERING_RULES.md`. For multi-layer user-facing changes, reviewers
must verify the applicable end-to-end path through source/ingestion, persistence,
API/service, frontend mapping/state, and rendered UI rather than treating one healthy
layer as proof of completeness. Passing the existing test suite alone is not
sufficient evidence that a stateful, scheduled, or multi-layer user-facing workflow
is safe to merge.
