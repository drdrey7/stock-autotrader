/**
 * Versioned constituent snapshot used by the publication boundary.
 *
 * Sources fetched on 2026-08-11:
 * - https://github.com/datasets/s-and-p-500-companies/blob/main/data/constituents.csv
 * - https://en.wikipedia.org/wiki/List_of_NASDAQ-100_companies
 *
 * Refresh this manifest as an explicit change when index membership changes.
 * Unknown symbols fail closed until the snapshot is refreshed.
 */
export const briefingUniverseMembershipVersion = "2026-08-11" as const;

export const briefingUniverseMembers = {
  "S&P 500": [
  ...
  "DPZ", "DRI", "DTE", "DUK", "DVA", "DVN", "DXCM", "EBAY", "ECHO", "ECL", "ED", "EFX",
  "EG", "EIX", "EL", "ELV", "EME", "EMR", "EOG", "EQIX", "EQR", "EQT", "ERIE", "ES",
  "ESS", "ETN", "ETR", "EVRG", "EW", "EXC", "EXE", "EXPD", "EXPE", "EXR", "F", "FANG",
  "FAST", "FCX", "FDS", "FDX", "FDXF", "FE", "FERG", "FFIV", "FICO", "FIS", "FISV", "FITB",
  ...
  "COST", "CPRT", "CRWD", "CRWV", "CSCO", "CSX", "CTAS", "DASH", "DDOG", "DXCM", "EXC", "FANG",
  "FAST", "FER", "FTNT", "GEHC", "GILD", "GOOG", "GOOGL", "HON", "HONA", "IDXX", "INTC", "INTU",
  "ISRG", "KDP", "KHC", "KLAC", "LIN", "LITE", "LRCX", "MAR", "MCHP", "MDLZ", "MELI", "META",
  "MNST", "MPWR", "MRVL", "MSFT", "MSTR", "MU", "NBIS", "NFLX", "NVDA", "NXPI", "ODFL", "ORLY",
  "PANW", "PAYX", "PCAR", "PDD", "PEP", "PLTR", "PYPL", "QCOM", "REGN", "RKLB", "ROP", "ROST",
  "SBUX", "SHOP", "SNDK", "SNPS", "SPCX", "STX", "TER", "TMUS", "TRI", "TSLA", "TTWO", "TXN",
  "VRTX", "WBD", "WDAY", "WDC", "WMT", "XEL"
  ],
} as const;

type DeclaredUniverse = keyof typeof briefingUniverseMembers;
const memberSets: Record<DeclaredUniverse, ReadonlySet<string>> = {
  "S&P 500": new Set(briefingUniverseMembers["S&P 500"]),
  "Nasdaq-100": new Set(briefingUniverseMembers["Nasdaq-100"]),
};

export function canonicalBriefingSymbol(symbol: string): string {
  return symbol.trim().toUpperCase().replace(/\./g, "-");
}

export function isBriefingSymbolInUniverse(symbol: string, universe: "S&P 500" | "Nasdaq-100" | "Both"): boolean {
  const canonical = canonicalBriefingSymbol(symbol);
  if (universe === "Both") {
    return memberSets["S&P 500"].has(canonical) && memberSets["Nasdaq-100"].has(canonical);
  }
  return memberSets[universe].has(canonical);
}
